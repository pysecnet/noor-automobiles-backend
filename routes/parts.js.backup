...
router.post('/upload', authenticateToken, isAdmin, upload.array('files', 5), (req, res) => {
  try {
    ...
    const fileUrls = req.files.map(file => ({
      url: `/uploads/parts/${file.filename}`,
...
router.get('/categories', (req, res) => {
  try {
    const categories = query.all('SELECT DISTINCT category FROM parts ORDER BY category');
...
router.get('/:id', (req, res) => {
  try {
    const part = query.get('SELECT * FROM parts WHERE id = ?', [req.params.id]);
...
router.post('/', authenticateToken, isAdmin, (req, res) => {
  ...
  try {
    ...
    const newPart = query.get('SELECT * FROM parts WHERE id = ?', [result.lastInsertRowid]);
...
router.put('/:id', authenticateToken, isAdmin, (req, res) => {
  ...
  const existingPart = query.get('SELECT id FROM parts WHERE id = ?', [partId]);
  if (!existingPart) {
    return res.status(404).json({ message: 'Part not found' });
  }
  ...
  try {
    ...
    query.run(`UPDATE parts SET ${updates.join(', ')} WHERE id = ?`, params);

    const updatedPart = query.get('SELECT * FROM parts WHERE id = ?', [partId]);
...
router.delete('/:id', authenticateToken, isAdmin, (req, res) => {
  ...
  try {
    const part = query.get('SELECT id FROM parts WHERE id = ?', [partId]);
    if (!part) {
      return res.status(404).json({ message: 'Part not found' });
    }

    query.run('DELETE FROM parts WHERE id = ?', [partId]);
